...
const suprTache=(key)=>{
setAllTask([...allTask.slice(0,key),...allTask.slice(key+1,allTask.length)])
...
<Routes>
<Route path="/"  element={<Taches   ChangeEtat={(value)=>ChangeEtat(value)}   handelClick={AfficherList} task={allTask} modifyListClick={(value)=>ModifyList(value)}  suprItem={(key)=>{suprTache(key)}} /> }/>

<Route path="/Modify" element={<Modify keys={keys} handelClickModifier={(value)=>{ModifierTache(value)}} task={allTask} />}/>

<Route path="/Ajouter" element={<Ajouter handelClickAfficher={(value)=>{AjouterTache(value)}}/>}/>
